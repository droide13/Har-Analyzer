...
def render_entry(entry: ParsedEntry, matched: bool, highlight_mode: bool, key_prefix: str) -> None:
    ...
    main_title = f"{status_emoji(entry.status)} {status_text} **{entry.method}** {safe_url}{cookie_badge_text}"
        ...
        with qp_tab:
            ...
            with col2:
                ...
                if entry.req_cookies or entry.res_cookies:
                    ...
                    if entry.req_cookies:
                        cookie_data["Request Cookies (sent to server)"] = _list_to_safe_dict(entry.req_cookies)
                    if entry.res_cookies:
                        cookie_data["Response Cookies (set by server)"] = _list_to_safe_dict(entry.res_cookies)
                    st.json(cookie_data)
                else:
...
if uploaded_file is not None:
    try:
        ...
        filtered_entries = [e for e in entries if entry_matches(e, filter_query, default_field, method_set)]
        
        # 2. Evaluate Highlighting Within Filtered List
        highlight_active = bool(highlight_query.strip())
        if highlight_active:
            match_flags = [entry_matches(e, highlight_query, default_field, set()) for e in filtered_entries]
        ...
        if highlight_active:
            ...
            for entry, is_matched in zip(filtered_entries[start_idx:end_idx], match_flags[start_idx:end_idx]):
                if is_matched:
